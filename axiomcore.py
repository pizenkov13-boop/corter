...
class AxiomConfig:
    ...
    @classmethod
    def from_mapping(cls, raw: Mapping[str, Any]) -> "AxiomConfig":
...
def load_config(path: Union[str, Path]) -> AxiomConfig:
    """Load YAML configuration into :class:`AxiomConfig`."""
    path = Path(path)
    with path.open("r", encoding="utf-8") as fh:
        raw = yaml.safe_load(fh) or {}
    if not isinstance(raw, dict):
        raise ValueError(f"Config root must be a mapping, got {type(raw).__name__}")
    return AxiomConfig.from_mapping(raw)
...
class AxiomDashboard:
    ...
    def _header_panel(self) -> Panel:
        title = Text("AxiomCore", style="bold magenta")
    ...
    def _footer_panel(self) -> Panel:
        ...
        return Panel(
            f"Best CV score: [bold green]{best}[/]  •  Refresh: {self.config.refresh_hz:.1f} Hz  •  axiomcore v{__version__}",
            style="dim",
        )
...
class AxiomCore:
    ...
    def __init__(
        ...
        self.dashboard = AxiomDashboard(config.tui, self.console)
        self.result_: Dict[str, Any] = {}

    @classmethod
    def from_yaml(cls, path: Union[str, Path], **kwargs: Any) -> "AxiomCore":
...
def _demo_config() -> str:
    return """\
# AxiomCore example configuration
task: auto
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="AxiomCore — Autonomous ML Optimization Framework")
    ...
    if args.config and args.config.exists():
        ...
        config = AxiomConfig.from_mapping(yaml.safe_load(_demo_config()))
    ...
    console = Console()
    console.print("\n[bold magenta]AxiomCore run complete[/]\n")
