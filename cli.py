#!/usr/bin/env python3
"""
AxiomCore CLI - Command Line Interface
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='AxiomCore - Autonomous ML Optimization Framework',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  axiomcore run data.csv                    # Run with default config.yaml
  axiomcore run data.csv -c custom.yaml     # Run with custom config
  axiomcore run data.csv --web              # Run with web dashboard
  axiomcore init                            # Create default config.yaml
  axiomcore version                         # Show version info
        """
    )
...
def run_optimization(args):
    ...
    print("⚡ AxiomCore - Autonomous ML Optimization")
    print("=" * 60)
    ...
    if not Path(args.config).exists():
        ...
        print(f"💡 Tip: Run 'axiomcore init' to create a default config")
        sys.exit(1)
    ...
    if args.web:
        ...
        web_thread = threading.Thread(target=lambda: run_server(debug=False), daemon=True)
        web_thread.start()
        print()
    
    try:
        # Load and run
        core = AxiomCore.from_yaml(args.config)
...
def create_default_config(args):
    ...
    default_config = """# AxiomCore Configuration
task: classification
...
"""
    ...
    print(f"   2. Run: axiomcore run data.csv")


def show_version():
    """Show version information"""
    from axiomcore import __version__
    print(f"AxiomCore v{__version__}")
    print("Autonomous ML Optimization Framework")
    print()
    print("GitHub: https://github.com/pizenkov13-boop/AxiomCore")
...
def start_web_dashboard(args):
    ...
    print("🚀 Starting AxiomCore Web Dashboard")
